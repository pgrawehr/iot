...
namespace Ads1115.Samples
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.Clear();
                Console.WriteLine("This triggers an interrupt when the value of AIN1 exceeds 1V");
                using (Iot.Device.Ads1115.Ads1115 adc = new Iot.Device.Ads1115.Ads1115(device, InputMultiplexer.AIN1, MeasuringRange.FS4096, DataRate.SPS860))
                {
                    ...
                    adc.EnableComparator(adc.VoltageToRaw(0.5), adc.VoltageToRaw(1.0), ComparatorMode.Traditional, ComparatorQueue.AssertAfterOne, controller, 23);
                    // (Do something else, here we print the output (as the console operations use to much time in the interrupt callback)
                    while (Console.KeyAvailable == false)
                    {
                        int interruptsThisPeriod = totalInterruptsSeen - previousNumberOfInterrupts;
