...
namespace Iot.Device.LEDMatrix
{
    ...
    public class RGBLedMatrix
    {
        ...
        public unsafe void DrawBitmap(int x, int y, Image<Rgb24> bitmap, bool backBuffer = false)
        {
            ...
            {
                Span<Rgb24> line = bitmap.GetPixelRowSpan(j);
                Span<byte> span = MemoryMarshal.Cast<Rgb24, byte>(line);
        ...
        /// <param name="backBuffer">true if want use back buffer, false otherwise</param>
        public unsafe void DrawBitmap(int x, int y, Image<Rgb24> bitmap, byte red, byte green, byte blue, byte repRed,
