...
namespace Iot.Device.Imu
{
    ...
    public class Mpu6050 : IDisposable
    {
        ...
        /// <summary>
        /// Represents the state of the data ready interrupt flag.
        /// This value is reset once the register is read.
        /// </summary>
        public bool DataReady
        {
            get
            {
                return (ReadByte(Register.INT_STATUS) & 0x01) == 0x01;
            }
        }
        ...
        /// <summary>
        /// Gets the sample rate for all sensors
        /// </summary>
        public float SampleRate
        {
            get
            {
                if (GyroscopeBandwidth == Mpu6050GyroBandwidth.BandWidth256Hz)
                {
                    return 8000.0f / (1.0f + SampleRateDivider);
                }

                return 1000.0f / (1.0f + SampleRateDivider);
            }
        }
