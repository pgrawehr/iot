...
namespace Iot.Device.Ft4222
{
    ...
    public class Ft4222Spi : SpiDevice
    {
        ...
        public Ft4222Spi(SpiConnectionSettings settings)
        {
            ...
            if (ftStatus != FtStatus.Ok)
            {
                throw new IOException(
                    $"Failed set clock rate {ft4222Clock} on device: {DeviceInformation.Description}with error: {ftStatus}");
            }
            ...
            {
                throw new IOException(
                    $"Failed setup SPI on device: {DeviceInformation.Description} with error: {ftStatus}");
            }
        }
        ...
        public override void TransferFullDuplex(ReadOnlySpan<byte> writeBuffer, Span<byte> readBuffer)
        {
            ...
            {
                throw new IOException(
                    $"{nameof(TransferFullDuplex)} failed to do a full duplex transfer, error: {ftStatus}");
            }
        }
