...
namespace Units
{
    ...
    public class GeodeticCalculations
    {
        ...
        private const int GEODESIC_ORDER = 6;
        private const int nC = GEODESIC_ORDER + 1;
        ...
        private static readonly double degree;

        private static readonly double NaN;

        private static readonly double tiny;

        private static readonly double tol0;

        private static readonly double tol1;

        private static readonly double tol2;

        private static readonly double tolb;

        private static readonly double xthresh;
...
/* Replacements for C99 math functions */
            ...
            GeodeticMask caps)
        {
            ...
            l.caps = (caps != GeodeticMask.GEOD_NONE
                         ? caps
                         : GeodeticMask.GEOD_DISTANCE_IN | GeodeticMask.GEOD_LONGITUDE) |
...
/* norm2(somg1, comg1); -- don't need to normalize! */
            ...
            if (l.caps & CAP_C1)
            {
                ...
                l.A1m1 = A1m1f(eps);
                C1f(eps, l.C1a);
                l.B11 = SinCosSeries(true, l.ssig1, l.csig1, l.C1a, GEODESIC_ORDER);
                s = Math.Sin(l.B11);
                c = Math.Cos(l.B11);
/* tau1 = sig1 + B11 */
...
/* tau2 = tau1 + tau12 */
                ...
                if (Math.Abs(l.f) > 0.01)
                {
                    ...
                    B12 = SinCosSeries(true, ssig2, csig2, l.C1a, GEODESIC_ORDER);
            ...
            if (outmask & (GeodeticMask.GEOD_DISTANCE | GeodeticMask.GEOD_REDUCEDLENGTH |
 ...
 * instead of Math.Sqrt(epsilon). */
...

            {
                ...
                if (calp0 != 0 && salp0 != 0)
                {
                    ...
                    C4f(g, eps, Ca);
                    B41 = SinCosSeries(false, ssig1, csig1, Ca, GEODESIC_ORDER);
                    B42 = SinCosSeries(false, ssig2, csig2, Ca, GEODESIC_ORDER);
                    S12 = A4 * (B42 - B41);
                }
                else
...
/* Convert -0 to 0 */
        ...
        private static void Lengths(GeodesicEllipsoidParams g,
 ...
 * and m0 = coefficient of secular term in expression for reduced length. */
            ...
            if (ps12b)
            {
                double B1 = SinCosSeries(true, ssig2, csig2, Ca, GEODESIC_ORDER) -
                            SinCosSeries(true, ssig1, csig1, Ca, GEODESIC_ORDER);
        ...
        private static double InverseStart(GeodesicEllipsoidParams g,
...
/* Update spherical estimate of alp1 using omg12 instead of lam12 */
        ...
        private static double Lambda12(GeodesicEllipsoidParams g,
            double sbet1, double cbet1, double dn1,
            double sbet2, double cbet2, double dn2,
...
/* sig12 = sig2 - sig1, limit to [0, pi] */
                ...
                somg12 = maxx(0, comg1 * somg2 - somg1 * comg2);
...
/* eta = omg12 - lam120 */
                ...
                eps = k2 / (2 * (1 + Math.Sqrt(1 + k2)) + k2);
                C3f(g, eps, Ca);
                B312 = (SinCosSeries(true, ssig2, csig2, Ca, GEODESIC_ORDER - 1) -
                        SinCosSeries(true, ssig1, csig1, Ca, GEODESIC_ORDER - 1));
                ...
                if (diffp)
                {
                    ...
                    {
                        Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2,
                            cbet1, cbet2, out _, out dlam12, out _, out _, out _, Ca);
                ...
                {
                    /* Evaluate A3 */
                    return polyval(GEODESIC_ORDER - 1, g.A3x, eps);
                }
                ...
                void C3f(GeodesicEllipsoidParams g, double eps, double[] c)
                {
                    ...
                    {
                        /* l is index of C3[l] */
                        int m = GEODESIC_ORDER - l - 1; /* order of polynomial in eps */
                        mult *= eps;
                        c[l] = mult * polyval(m, SubArray(g.C3x, o), eps);
                        o += m + 1;
                    }
                }
                ...
                void C4f(GeodesicEllipsoidParams g, double eps, double[] c)
                {
                    ...
                    double mult = 1;
                    int o = 0, l;
                    for (l = 0; l < GEODESIC_ORDER; ++l)
                    {
                        /* l is index of C4[l] */
                        int m = GEODESIC_ORDER - l - 1; /* order of polynomial in eps */
                        c[l] = mult * polyval(m, SubArray(g.C4x, o), eps);
                        o += m + 1;
                        mult *= eps;
                    }
                }

                private double[] SubArray(double[] array, int fromIndex)
                {
                    var ret = new double[array.Length - fromIndex];
                    Array.Copy(array, fromIndex, ret, 0, array.Length - fromIndex);
                    return ret;
                }
...
/* The coefficients C1[l] in the Fourier expansion of B1 */
                void C1f(double eps, double[] c)
                {
                    ...
                    {
                        /* l is index of C1p[l] */
                        int m = (GEODESIC_ORDER - l) / 2; /* order of polynomial in eps^2 */
                        c[l] = d * polyval(m, SubArray(coeff, o), eps2) / coeff[o + m + 1];
                        o += m + 2;
                        d *= eps;
                    }
                }
...
/* The scale factor A3 = mean value of (d/dsigma)I3 */
                void A3coeff(GeodesicEllipsoidParams g)
                {
                    ...
                    {
                        /* coeff of eps^j */
                        int m = GEODESIC_ORDER - j - 1 < j
                            ? GEODESIC_ORDER - j - 1
                            : j; /* order of polynomial in n */
                        g.A3x[k++] = polyval(m, SubArray(coeff, o), g.n) / coeff[o + m + 1];
                        o += m + 2;
                    }
                }
...
/* The coefficients C3[l] in the Fourier expansion of B3 */
                void C3coeff(GeodesicEllipsoidParams g)
                {
                    ...
                    {
                        /* l is index of C3[l] */
                        for (j = GEODESIC_ORDER - 1; j >= l; --j)
                        {
                            /* coeff of eps^j */
                            int m = GEODESIC_ORDER - j - 1 < j
                                ? GEODESIC_ORDER - j - 1
                                : j; /* order of polynomial in n */
                            g.C3x[k++] = polyval(m, SubArray(coeff, o), g.n) / coeff[o + m + 1];
                            o += m + 2;
                        }
                    }
                }
...
/* The coefficients C4[l] in the Fourier expansion of I4 */
                void C4coeff(GeodesicEllipsoidParams g)
                {
                    ...
                    for (l = 0; l < GEODESIC_ORDER; ++l)
                    {
                        ...
                        {
                            /* coeff of eps^j */
                            int m = GEODESIC_ORDER - j - 1; /* order of polynomial in n */
                            g.C4x[k++] = polyval(m, SubArray(coeff, o), g.n) / coeff[o + m + 1];
                            o += m + 2;
                        }
                    }
                }
