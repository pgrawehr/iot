...
void geod_setdistance(geod_geodesicline l, double s13) {
  l.s13 = s13;
...
void geod_gensetdistance(geod_geodesicline l,
...
double geod_gendirect(geod_geodesic g,
                    double lat1, double lon1, double azi1,
                    uint flags, double s12_a12,
                    out double plat2, out double plon2, out double pazi2,
                    out double ps12, ref double? pm12, ref double? pM12, ref double? pM21,
                    ref double? pS12) {
  geod_geodesicline l;
  uint outmask = GEOD_LATITUDE | GEOD_LONGITUDE | GEOD_AZIMUTH | GEOD_DISTANCE |
                 (pm12 != null ? GEOD_REDUCEDLENGTH : GEOD_NONE) |
                 (pM12 != null || pM21 != null ? GEOD_GEODESICSCALE : GEOD_NONE) |
                 (pS12 != null ? GEOD_AREA : GEOD_NONE);
                 ...
                 out double plat2, out double plon2, out double pazi2)
{
  ...
  geod_gendirect(g, lat1, lon1, azi1, GEOD_NOFLAGS, s12, out plat2, out plon2, out pazi2,
                 out _, ref tmp, ref tmp, null, null);
}
...
bool redlp = pm12b || pm0 || pM12 || pM21;
  if (ps12b || redlp) {
    A1 = A1m1f(eps);
C1f(eps, Ca);
    if (redlp) {
...
double InverseStart(geod_geodesic g,
...
double Lambda12(geod_geodesic g,
              double sbet1, double cbet1, double dn1,
              double sbet2, double cbet2, double dn2,
...
double A3f(geod_geodesic g, double eps) {
  /* Evaluate A3 */
...
uint geod_polygon_testpoint(geod_geodesic g,
                                geod_polygon* p,
                                double lat, double lon,
                                bool reverse, bool sign,
                                double* pA, double* pP) {
...
uint geod_polygon_testedge(geod_geodesic g,
                               geod_polygon* p,
                               double azi, double s,
                               bool reverse, bool sign,
                               double* pA, double* pP) {
...
double areareduceA(double area[], double area0,
                 int crossings, bool reverse, bool sign)
{
    accrem(area, area0);
    if (crossings & 1)
