...
namespace Iot.Device.Bh1745
{
    ...
    public class Bh1745 : IDisposable
    {
        ...
        public Color GetCompensatedColor()
        {
            ...
            double compensatedRed = ReadRedDataRegister() * ChannelCompensationMultipliers.Red /
                                    MeasurementTime.ToMilliseconds() * 360;
            double compensatedGreen = ReadGreenDataRegister() * ChannelCompensationMultipliers.Green /
                                      MeasurementTime.ToMilliseconds() * 360;
            double compensatedBlue = ReadBlueDataRegister() * ChannelCompensationMultipliers.Blue /
                                     MeasurementTime.ToMilliseconds() * 360;
            double compensatedClear = clearDataRaw * ChannelCompensationMultipliers.Clear /
                                      MeasurementTime.ToMilliseconds() * 360;
        ...
        private void InitDevice()
        {
            ...
            {
                throw new Exception(
                    $"Manufacturer ID {ManufacturerId} is not the same as expected 224. Please check if you are using the right device.");
            }

            if (PartId != 0x0b)
            {
                throw new Exception(
                    $"Part ID {PartId} is not the same as expected 11. Please check if you are using the right device.");
            }
